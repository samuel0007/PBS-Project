...
BOUNDS = 3. # If particles are falling down, change this to int.
REST_DENSITY = 300
RADIUS = 0.025
...
# simulation = Simulation(NUM_PARTICLES, MAX_TIME, max_dt=MAX_DT, mass=MASS, rest_density=REST_DENSITY, support_radius=SUPPORT_RADIUS, mu=MU, b_mu=B_MU, bounds=BOUNDS, is_frame_export=True, debug=True, result_dir=RESULT_DIR, pointData_file=r"src/pointDataMetaFiles/test.txt", boundary_pointData_file = r"", is_uniform_export=True)
# simulation.run()

# Render Simulation
renderer = Renderer(result_dir=RESULT_DIR, radius=RADIUS*0.99, SHOW=True, render_boundary=True, render_density=True, mass=MASS, start_frame=0, render_uniform=False)
renderer.render()
